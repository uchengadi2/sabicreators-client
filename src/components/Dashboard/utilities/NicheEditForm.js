...
import { CREATE_LANGUAGE, CREATE_LOCATION, CREATE_NICHE, EDIT_NICHE } from "../../../actions/types";
...
function NicheEditForm(props) {
 const { params, token, userId } = props;
  ...
  const onSubmit = (formValues) => {
    ...
    form.append("niche", formValues.niche ? formValues.niche: params[0].niche);
    ...
    if (formValues) {
              const createForm = async () => {
                ...
                const response = await api.patch(`/niches/${params[0].id}`, form);
                ...
                if (response.data.status === "success") {
                  ...
                  props.handleSuccessfulEditSnackbar(
                    `${response.data.data.data.niche} Niche is updated successfully!!!`
                  );
                  props.renderNicheEdittedUpdateCounter();
