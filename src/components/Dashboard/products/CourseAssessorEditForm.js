...
import { EDIT_COURSE_ASSESSOR } from "../../../actions/types";
...
function CourseAssessorEditForm(props) {
  ...
  const [assessor, setAssessor] = useState(params[0].assessorId);
  ...
  const onSubmit = (formValues) => {
    ...
    if (!assessor || assessor.replace(/\s/g, "").length === 0) {
      props.handleFailedSnackbar("Select the Assessor ");
      setLoading(false);
      return;
    }
    ...
    form.append("assessor", assessor ? assessor : params[0].assessor);
    ...
    if (formValues) {
      const editForm = async () => {
        ...
        const response = await api.patch(
          `/courseassessors/${params[0].id}`,
          form
        );

        if (response.data.status === "success") {
          dispatch({
            type: EDIT_COURSE_ASSESSOR,
            payload: response.data.data.data,
          });

          props.handleSuccessfulEditSnackbar(
            `${assessorName} Information on this course is updated successfully!!!`
          );
