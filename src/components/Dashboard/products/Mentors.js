...
import MentorDeleteForm from "./MentorDeleteForm";
...
function Mentors(props) {
    ...
    const fetchData = async () => {
      ...
      workingData.map((mentor) => {
        allData.push({
          ...
          slug: mentor.slug,
          accountDetails: mentor.accountDetails,
          image: mentor.image,
          user: mentor.user,
  ...
  const renderDataGrid = () => {
    ...
    categoriesList.map((mentor, index) => {
      ...
      let row = {
        ...
        image: mentor.image,
        slug: mentor.slug,
        name: mentor.name.replace(
          /(^\w|\s\w)(\S*)/g,
          (_, m1, m2) => m1.toUpperCase() + m2.toLowerCase()
        ),
        description: mentor.description.replace(
          /(^\w|\s\w)(\S*)/g,
          (_, m1, m2) => m1.toUpperCase() + m2.toLowerCase()
        ),
        ...
        gitHubProfile: mentor.gitHubProfile,
        bio: mentor.bio,
        userId: mentor.user.length === 1 ? mentor.user[0].id : " ",
      };
      rows.push(row);
    });
