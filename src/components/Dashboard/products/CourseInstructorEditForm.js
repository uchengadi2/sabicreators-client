...
function CourseInstructorEditForm(props) {
  ...
  const [instructor, setInstructor] = useState(params[0].instructorId);
  ...
  const onSubmit = (formValues) => {
    ...
    form.append("instructor", instructor ? instructor : params[0].instructor);
    ...
    form.append(
      "homeHourlyRate",
      formValues.homeHourlyRate
        ? formValues.homeHourlyRate
        : params[0].homeHourlyRate
    );
    form.append(
      "trackPreference",
      trackPreference ? trackPreference : params[0].trackPreference
    );
    form.append(
      "deliveryPreference",
    ...
    form.append(
      "acceptHomeTeaching",
      acceptHomeTeaching ? acceptHomeTeaching : params[0].acceptHomeTeaching
    );
    ...
    if (formValues) {
      const editForm = async () => {
        ...
        const response = await api.patch(
          `/courseinstructors/${params[0].id}`,
          form
        );

        if (response.data.status === "success") {
          dispatch({
            type: EDIT_COURSE_INSTRUCTOR,
            payload: response.data.data.data,
          });

          props.handleSuccessfulEditSnackbar(
            `${instructorName} Information on this course is updated successfully!!!`
          );
    ...
    <form id="addInstructorForm">
      ...
      >
        ...
        <Field
          ...
          name="homeHourlyRate"
          type="number"
          defaultValue={params[0].homeHourlyRate}
