...
import AssessorEditForm from "./AssessorEditForm";
import AssessorDeleteForm from "./AssessorDeleteForm";
...
function Assessors(props) {
    ...
    const fetchData = async () => {
      ...
      workingData.map((assessor) => {
        allData.push({
          ...
          refNumber: assessor.refNumber,
          description: assessor.description,
          qualifications: assessor.qualifications,
          skills: assessor.skills,
          experiences: assessor.experiences,
          status: assessor.status,
          slug: assessor.slug,
          accountDetails: assessor.accountDetails,
          image: assessor.image,
          user: assessor.user,
          dateCreated: assessor.dateCreated,
          linkedInProfile: assessor.linkedInProfile,
          gitHubProfile: assessor.gitHubProfile,
          bio: assessor.bio,
        });
      });
  ...
  const renderDataGrid = () => {
    ...
    categoriesList.map((assessor, index) => {
      ...
      let row = {
        ...
        image: assessor.image,
        slug: assessor.slug,
        name: assessor.name.replace(
          /(^\w|\s\w)(\S*)/g,
          (_, m1, m2) => m1.toUpperCase() + m2.toLowerCase()
        ),
        description: assessor.description.replace(
          /(^\w|\s\w)(\S*)/g,
          (_, m1, m2) => m1.toUpperCase() + m2.toLowerCase()
        ),
        ...
        gitHubProfile: assessor.gitHubProfile,
        bio: assessor.bio,
        userId: assessor.user.length === 1 ? assessor.user[0].id : " ",
      };
      rows.push(row);
    });
      ...
      <Grid container spacing={1} direction="column">
        <Grid item xs>
          <Grid container spacing={2}>
            <Grid item xs={9.3}>
              {/* <Item>xs=8</Item> */}
              <Typography variant="h4">Assessors</Typography>
            </Grid>
            <Grid item xs={2.7}>
              <div>
