...
import AddNewProgramme from "./AddNewProgramme";
import ProgrammeEditForm from "./ProgrammeEditForm";
import ProgrammeDeleteForm from "./ProgrammeDeleteForm";
...
function Programmes(props) {
    ...
    const fetchData = async () => {
      ...
      workingData.map((programme) => {
        allData.push({
          ...
          slug: programme.slug,
          channelId: programme.channel[0].id,
  ...
  const renderDataGrid = () => {
    ...
    programmesList.map((programme, index) => {
      console.log("proramme is:", programme);
      let row = {
        numbering: ++counter,
        id: programme.id,
        image: programme.image,
        name: programme.name.replace(
          /(^\w|\s\w)(\S*)/g,
          (_, m1, m2) => m1.toUpperCase() + m2.toLowerCase()
        ),
        description: programme.description.replace(
          /(^\w|\s\w)(\S*)/g,
          (_, m1, m2) => m1.toUpperCase() + m2.toLowerCase()
        ),
        channel: programme.channel,
