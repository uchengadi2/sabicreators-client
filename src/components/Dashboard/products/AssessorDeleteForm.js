...
function AssessorDeleteForm(props) {
  ...
  const handleDelete = () => {
    ...
    if (params[0].id) {
      const editForm = async () => {
        ...
        const response = await api.delete(`/assessors/${params[0].id}`);

        if (response.status === 204) {
          props.handleSuccessfulDeletedItemSnackbar(
            `Assessor, ${params[0].name}  is deleted successfully!!!`
          );
  ...
  return (
    <>
      ...
      >
        <AlertTitle>Delete Assessor "{params[0].name}" ?</AlertTitle>
...
export default AssessorDeleteForm;
