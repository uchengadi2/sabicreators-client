...
import { EDIT_ASSESSOR } from "../../../actions/types";
...
function AssessorEditForm(props) {
    ...
    return (
      <Box>
        <FormControl variant="outlined">
          ...
          <FormHelperText>Connect Assessor to a Registered User</FormHelperText>
        </FormControl>
      </Box>
    );
  };
  ...
  const onSubmit = (formValues) => {
    ...
    if (formValues) {
      const editForm = async () => {
        ...
        const response = await api.patch(`/assessors/${params[0].id}`, form);
        ...
        if (response.data.status === "success") {
          ...
          props.handleSuccessfulEditSnackbar(
            `Assessor, ${response.data.data.data.name} Information is updated successfully!!!`
          );
