...
import { EDIT_COURSE_LESSON } from "../../../actions/types";
...
function CourseLessonEditForm(props) {
    ...
    if (formValues) {
      const editForm = async () => {
        ...
        const response = await api.patch(
          `/courselessons/${params[0].id}`,
          form
        );

        if (response.data.status === "success") {
          dispatch({
            type: EDIT_COURSE_LESSON,
            payload: response.data.data.data,
          });

          props.handleSuccessfulEditSnackbar(
            `${response.data.data.data.title} lesson is updated successfully!!!`
          );
