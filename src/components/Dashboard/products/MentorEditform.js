...
import { EDIT_ASSESSOR, EDIT_MENTOR } from "../../../actions/types";
    ...
    return (
      <Box>
        <FormControl variant="outlined">
          ...
          <FormHelperText>Connect Mentor to a Registered User</FormHelperText>
        </FormControl>
      </Box>
    );
  };
  ...
  const onSubmit = (formValues) => {
    ...
    if (formValues) {
      const editForm = async () => {
        ...
        const response = await api.patch(`/mentors/${params[0].id}`, form);
        ...
        if (response.data.status === "success") {
          ...
          props.handleSuccessfulEditSnackbar(
            `Mentor, ${response.data.data.data.name} Information is updated successfully!!!`
          );
