...
import AddCourseAssessorForm from "./AddCourseAssessorForm";
import CourseAssessorDeleteForm from "./CourseAssessorDeleteForm";
import CourseAssessorEditForm from "./CourseAssessorEditForm";
...
function CourseAssessors(props) {
  ...
  const [courseAssessorsList, setCourseAssessorsList] = useState([]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const response = await api.get(`/courseassessors`);
      ...
      setCourseAssessorsList(allData);
      setLoading(false);
    };
  ...
  const renderDataGrid = () => {
    ...
    courseAssessorsList.map((course, index) => {
      let row = {
        numbering: ++counter,
        id: course.id,
        refNumber: course.refNumber,
        assessor: course.assessor,
        course: course.dcourse,
        channel: course.channel,
        programme: course.programme,
        hourlyRate: course.hourlyRate,
        minimumAllowableHours: course.minimumAllowableHours,
        ...
        assessorName:
          course.assessor.length === 1 ? course.assessor[0].name : " ",
        ...
        assessorId: course.assessor[0].id,
      ...
      <Grid container spacing={1} direction="column">
        <Grid item xs>
          <Grid container spacing={2}>
            <Grid item xs={9.3}>
              {/* <Item>xs=8</Item> */}
              <Typography variant="h4">Course Assessors</Typography>
            </Grid>
            <Grid item xs={2.7}>
              <div>
...
export default CourseAssessors;
