...
function MarkOrderAsCompleted(props) {
  ...
  const handleMarkAsCompleted = () => {
    ...
    const data = {
        ...
        markForCompletionBy: userId,
        markForCompletionDate: new Date().toISOString(),
        markedByIdentity: "brand",
    };
      ...
      const editForm = async () => {
        ...
        if (response.data.status === "success") {
          props.handleSuccessfulDeletedItemSnackbar(
            `${params[0].projectName} project is successfully marked as completed!!!`
          );
  ...
  return (
    <>
      ...
      >
        <AlertTitle>Mark "{params[0].projectName}" Project  as Completed" ?</AlertTitle>
...
export default MarkOrderAsCompleted;
