...
function AddNewProjectForm(props) {
  ...
  const onSubmit = (formValues) => {
    ...
    if (
      ...
      props.handleFailedSnackbar("The project name field cannot be empty");
      setLoading(false);
      return;
    }
    ...
    if (
      !formValues["brief"] ||
      formValues["brief"].replace(/\s/g, "").length === 0
    ) {
      props.handleFailedSnackbar(
        "Please providee the project brief"
      );
      setLoading(false);
      return;
    }
    ...
    form.append("brief", formValues.brief);
    form.append("brand", props.brandId);
    //form.append("creator", "67c8856e3295de3924761490");
    ...
    if (formValues) {
      const createForm = async () => {
        ...
        if (response.data.status === "success") {
          ...
          props.renderProjectUpdateCounter();
