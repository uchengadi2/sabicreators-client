...
function AddCourseMentorForm(props) {
    ...
    return (
      <Box>
        <FormControl variant="outlined">
          ...
          <FormHelperText>
            Select Mentor Delivery Platform Preference
          </FormHelperText>
        </FormControl>
      </Box>
    );
  ...
  const onSubmit = (formValues) => {
    ...
    if (!formValues["refNumber"]) {
      const refNumber =
        "NEXT" + "-" + Math.floor(Math.random() * 100000000) + "-" + "MENTOR";
    ...
    if (formValues) {
      const createForm = async () => {
        api.defaults.headers.common["Authorization"] = `Bearer ${props.token}`;
        const response = await api.post(`/coursementors`, form);

        if (response.data.status === "success") {
          dispatch({
            type: CREATE_COURSE_MENTOR,
            payload: response.data.data.data,
          });

          props.handleSuccessfulCreateSnackbar(
            ` ${mentorName} is added as a Mentor on this course!!!`
          );
