...
import CourseTopicDeleteForm from "./CourseTopicDeleteForm";
import AddCourseTopicForm from "./AddCourseTopicForm";
import CourseTopicEditForm from "./CourseTopicEditForm";
...
function CourseTopics(props) {
  ...
  const [courseTopicsList, setCourseTopicsList] = useState([]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const response = await api.get(`/coursetopics`);
      ...
      workingData.map((dcourse) => {
        allData.push({
          ...
          lesson: dcourse.lesson,
          title: dcourse.title,
          status: dcourse.status,
          description: dcourse.description,
          videoId: dcourse.videoId,
          createdBy: dcourse.createdBy,
          dateCreated: dcourse.dateCreated,
          sequenceNumber: dcourse.sequenceNumber,
        });
      });
      setCourseTopicsList(allData);
      setLoading(false);
    };
  ...
  const renderDataGrid = () => {
    ...
    courseTopicsList.map((course, index) => {
      ...
      <Grid container spacing={1} direction="column">
        <Grid item xs>
          <Grid container spacing={2}>
            <Grid item xs={9.3}>
              {/* <Item>xs=8</Item> */}
              <Typography variant="h4">Course Topics</Typography>
            </Grid>
            <Grid item xs={2.7}>
              <div>
