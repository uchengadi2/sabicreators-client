...
import { CREATE_ASSESSOR } from "../../../actions/types";
...
function AddAssessorForm(props) {
  ...
  const onSubmit = (formValues) => {
    ...
    if (formValues) {
      const createForm = async () => {
        api.defaults.headers.common["Authorization"] = `Bearer ${props.token}`;
        const response = await api.post(`/assessors`, form);

        if (response.data.status === "success") {
          dispatch({
            type: CREATE_ASSESSOR,
            payload: response.data.data.data,
          });

          props.handleSuccessfulCreateSnackbar(
            `${response.data.data.data.name} assessor is added successfully!!!`
          );
