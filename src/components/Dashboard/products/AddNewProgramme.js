...
import { CREATE_PROGRAMME } from "../../../actions/types";
...
function AddNewProgramme(props) {
  const classes = useStyles();

  const [loading, setLoading] = useState(false);
  const [channel, setChannel] = useState();
  const [channelsList, setChannelsList] = useState([]);
  const [programmeClass, setProgrammeClass] = useState("public");
  ...
  const onSubmit = (formValues) => {
    ...
    if (
      ...
      props.handleFailedSnackbar("The programme name field cannot be empty");
      setLoading(false);
      return;
    }
    ...
    if (
      ...
      props.handleFailedSnackbar(
        "The programme description field cannot be empty"
      );
      setLoading(false);
      return;
    }
    ...
    form.append("status", programmeStatus);
    form.append("channel", channel);
    form.append("track", track);
    form.append("class", programmeClass);
    ...
    if (formValues) {
      const createForm = async () => {
        api.defaults.headers.common["Authorization"] = `Bearer ${props.token}`;
        const response = await api.post(`/programmes`, form);
        ...
        if (response.data.status === "success") {
          ...
          props.handleSuccessfulCreateSnackbar(
            `${response.data.data.data.name} Programme is created successfully!!!`
          );
